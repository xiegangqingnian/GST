...
echo "正在停止浏览器服务..."
pkill node$
\cp -f /work/node10.13.0/node /usr/bin/node
\cp -f /work/node10.13.0/node /usr/sbin/node
\cp -f /work/node10.13.0/node /usr/local/bin/node
...
chown -R mongod:mongod /work/gst_install/monitor/mongo
chmod -R 777 /work/gst_install/monitor/mongo
