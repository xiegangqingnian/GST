...
static int64_t reward_vote = 2'000'000'0000.0; //    ��Ϣ���㷽�� ��Ѻ*ʱ��/reward_vote
static int64_t reward_max = 100000000;		   //��Ϳ�ȡ��Ϣֵ// 0.00000432*100000000  ��Ϣֵ�Ŵ���10��8�η�   1gst 1�� 0.0000'0432
							   ...
							   const asset stake_net_delta, const asset stake_cpu_delta, bool transfer)
{
	...
	gstio_assert(std::abs((stake_net_delta + stake_cpu_delta).amount) >= std::max(std::abs(stake_net_delta.amount), std::abs(stake_cpu_delta.amount)),
	...
	// update stake delegated from "from" to "receiver"���µ�Ѻ��¼��
	{
	...
	if (N(gstio.stake) != source_stake_from)
	{ //for gstio both transfer and refund make no sense
		...
		if (asset(0) < transfer_amount)
		{
			...
			(N(gstio.token), {source_stake_from, N(active)},
...
void system_contract::refund(const account_name owner)
{
	...
	INLINE_ACTION_SENDER(gstio::token, transfer)
	(N(gstio.token), {N(gstio.stake), N(active)},
