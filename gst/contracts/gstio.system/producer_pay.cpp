...
namespace gstiosystem {
	...
	void system_contract::claimrewards(const account_name& owner) {
		...
		if (usecs_since_last_fill > 0 && _gstate.last_pervote_bucket_fill > 0) {
			...
			auto to_per_del_pay = to_producers / 10; //增发10%用于支付投票利息
			auto to_per_vote_pay = to_producers - to_per_block_pay - to_per_del_pay;

			INLINE_ACTION_SENDER(gstio::token, issue)(N(gstio.token), { {N(gstio),N(active)} },
				{ N(gstio), asset(new_tokens), std::string("issue tokens for producer pay and savings") });

			INLINE_ACTION_SENDER(gstio::token, transfer)(N(gstio.token), { N(gstio),N(active) },
			...
			INLINE_ACTION_SENDER(gstio::token, transfer)(N(gstio.token), { N(gstio),N(active) },
				{ N(gstio), N(gstio.vote), asset(to_per_del_pay), "fund per-vote bucket" });    //增发转给gstio.vote用于支付投票利息
		...
		if (producer_per_block_pay > 0) {
			INLINE_ACTION_SENDER(gstio::token, transfer)(N(gstio.token), { N(gstio.bpay),N(active) },
				{ N(gstio.bpay), owner, asset(producer_per_block_pay), std::string("producer block pay") });
		}
		if (producer_per_vote_pay > 0) {
			INLINE_ACTION_SENDER(gstio::token, transfer)(N(gstio.token), { N(gstio.vpay),N(active) },
